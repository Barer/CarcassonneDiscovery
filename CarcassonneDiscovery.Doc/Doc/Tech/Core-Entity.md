# CarcassonneDiscovery.Entity.dll

V této knihovnì se nachází objekty reprezentující herní prvky.
...
Výètový typ `PlayerColor` reprezentuje barvu figurek hráèe. Pomocí barev jsou hráèi identifikováni pro úèely hry. Zvláštní barva `PlayerColor.None` je použita pøi akcích, které nejsou iniciovány žádným hráèem.
...
Rozhraní `ITileScheme` reprezentuje vzhled kartièky. Vzhled kartièek je urèen poètem regionù a mìst a jejich strukturou (sousednosti).

Objekty implementující `ITileScheme` umožòují:
